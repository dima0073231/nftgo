...
// Обробник запуску гри з подарунком
window.addEventListener('startGiftGame', () => {
  // Додаткова логіка перед запуском гри, якщо потрібно
  startGame();
});

// Обробник успішного кешауту подарунка
window.addEventListener('giftCashoutSuccess', (e) => {
  const { winAmount } = e.detail;
  console.log(`Гравець виграв ${winAmount} TON на подарунку`);
});